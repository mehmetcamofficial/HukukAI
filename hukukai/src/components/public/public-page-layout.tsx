...
export function PublicHeader({ active }: { active?: 'product' | 'services' | 'about' | 'announcements' | 'contact' }) {
  ...
  return <header className="landing-nav public-header" ref={ref}><div className="brand-wrapper"><Link href="/" className="landing-logo"><span><Scale size={17} /></span>Hukuk<em>AI</em></Link></div><nav className={`landing-nav-links ${mobileOpen ? 'open' : ''}`} aria-label="Ana gezinme">{group('Ürün', productItems, active === 'product')}{group('Dava Yönetimi', caseItems, active === 'product')}<Link href="/hizmet-alanlari" className={active === 'services' ? 'public-nav-active' : ''} onClick={close}>Hizmet Alanları</Link><Link href="/hakkinda" className={active === 'about' ? 'public-nav-active' : ''} onClick={close}>Hakkında</Link><Link href="/duyurular" className={active === 'announcements' ? 'public-nav-active' : ''} onClick={close}>Duyurular</Link><Link href="/iletisim" className={active === 'contact' ? 'public-nav-active' : ''} onClick={close}>İletişim</Link><Link href="/login" className="nav-demo-mobile" onClick={close}>Demoyu İncele <ArrowRight size={14} /></Link></nav><Link href="/login" className="nav-cta">Demoyu İncele <ArrowRight size={14} /></Link><button type="button" className="menu-toggle" aria-label="Menüyü aç veya kapat" aria-expanded={mobileOpen} onClick={() => { setMobileOpen((v) => !v); setOpenGroup(null); }}>{mobileOpen ? <X size={20} /> : <Menu size={20} />}</button></header>;
}
